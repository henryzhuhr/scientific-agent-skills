...
本参考内容涵盖了Vaex的性能特性，包括延迟求值、缓存、内存管理、异步操作以及处理海量数据集的优化策略。
...
Vaex 可以使用 Just-In-Time 编译进行自定义操作：
...
1. **使用 HDF5 或 Arrow 格式** - 比 CSV
2 快几个数量级。 **利用惰性计算** - 在必要时不要强制计算
3. **延迟 = True 的批量操作** - 最小化通过 data
4 的次数。 **保持列虚拟** - 仅在有益时实现
5. **使用选择而不是过滤器** - 对于多个段更有效
6. **分析您的代码** - 在优化
7之前识别瓶颈。 **避免 `.values` 和 `.to_pandas_df()`** - 在 Vaex
8 中保留操作。 **自然并行** - Vaex 自动使用所有内核
9. **导出为高效格式** - 检查点复杂管道
10. **优化表达式** - 简化数学并重用计算

## 相关资源

- DataFrame 基础知识：参见 `core_dataframes.md`
  - 数据操作：参见 `data_processing.md`
  - 文件 I/O 优化：参见 `io_operations.md`
