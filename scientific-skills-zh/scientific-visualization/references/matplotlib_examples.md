...
此参考提供了使用 Matplotlib、Seaborn 和 Plotly 创建可供出版的科学图形的实用代码示例。所有示例均遵循 `publication_guidelines.md` 的最佳实践，并使用 `color_palettes.md`.
...
### Seaborn
- 构建于matplotlib，因此所有 matplotlib 自定义都有效
- 使用 `sns.set_style('ticks')` 或 `'whitegrid'` 获得干净的外观
- `sns.despine()` 删除顶部和右侧的脊柱
- 使用 `sns.set_palette()`

### 设置自定义调色板Plotly
- 非常适合交互式探索性分析
- 使用 `fig.write_image()` 导出静态图像（需要 kaleido 包）
- 使用 `scale` 参数控制 DPI（比例 = 3 ≈ 300 DPI）
- 广泛更新布局以提高出版质量
...
4. **自定义颜色**（使用色盲友好的调色板）
5. **调整字体和线宽**（以最终尺寸可读）
6. **删除图表垃圾**（顶部/右侧的刺，过多的网格）
7. **添加带有单位的清晰标签**
8. **灰度测试**
9. **以多种格式保存**（PDF 用于矢量，PNG 用于光栅）
10. **在最终上下文中验证**（导入手稿以检查大小）
