...
## 中的优化目录
1. [内置优化器](#built-in-optimizers)
2. [梯度计算](#gradient-computation)
3. [变分算法](#variational-algorithms)
4. [QAOA](#qaoa-量子-近似优化-算法)
5. [训练策略](#training-strategies)
6. [优化挑战](#optimization-challenges)
...
1. **选择合适的优化器** - Adam用于一般用途，QNG用于变分电路
2. **在硬件上使用参数转换** - Backprop 仅适用于模拟器 
3. **小心初始化** - 通过智能初始化
4避免贫瘠的高原。 **监控梯度** - 检查梯度消失/爆炸
5. **使用学习率表** - 随着时间的推移衰减学习率
6. **尝试多次重新启动** - 逃避局部最小值
7. **在测试集上验证** - 防止过度拟合
8. **配置文件优化** - 识别瓶颈
9. **剪辑渐变** - 防止不稳定
10. **从浅层开始** - 最初使用较少的层，然后增长
