...
此参考文档记录了 pymatgen 的转换框架，并提供常见材料科学工作流程的秘诀。

## 转换框架

转换提供了一种系统的方法来修改结构，同时跟踪修改历史记录。
...
2. **错误处理**：检查文件是否存在以及计算收敛性
3. **文档**：使用 `TransformedStructure`
4 跟踪转换历史记录。 **版本控制**：将输入参数和脚本存储在git
5中。 **自动化**：将工作流程管理器（Fireworks、AiiDA）用于复杂的管道
6. **数据管理**：以清晰的目录结构
7组织计算。 **验证**：在继续之前始终验证中间结果
