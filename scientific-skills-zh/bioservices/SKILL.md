---
name: bioservices
description: 与 40 多种生物信息学服务的统一 Python 接口。在具有一致 API 的单个工作流程中查询多个数据库（UniProt、KEGG、ChEMBL、Reactome）时使用。最适合跨数据库分析、跨服务的 ID 映射。对于快速单数据库查找，请使用 gget；对于序列/文件操作，请使用 biopython。
...
- `retrieve()`：获取各种格式的蛋白质条目（FASTA、XML、tab）
- `mapping()`：在之间转换标识符数据库

参考：`references/services_reference.md`了解完整的UniProt API详细信息。
...
参考：`references/workflow_patterns.md`进行完整通路分析工作流程.
...
BioServices 擅长组合多种服务进行全面分析。常见集成模式：
...
- **制表符分隔（TSV）**：Pandas表格数据的数据帧
- **字典/JSON**：直接Python操作
- **FASTA**：用于序列分析的BioPython集成
...
- `services_reference.md`：所有 40 多种服务及方法的综合列表
- `workflow_patterns.md`：详细的多步骤分析工作流程
- `identifier_mapping.md`：跨数据库 ID 的完整指南转换
