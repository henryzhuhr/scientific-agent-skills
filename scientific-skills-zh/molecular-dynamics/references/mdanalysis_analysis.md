...
## MDAnalysis Universe 和 AtomGroup
...
|格式|扩展|注释 |
|--------|------------|--------|
| DCD | `.dcd` | CHARMM/NAMD 二进制；应用广泛|
| XTC | `.xtc` | GROMACS 压缩 |
| TRR | `.trr` | GROMACS全精度|
|网络CDF | `.nc` |琥珀色格式 |
|兰姆普斯 | `.lammpstrj` | LAMMPS转储|
| HDF5 | `.h5md` | H5MD标准|
| PDB| `.pdb` |多模型 PDB |
...
- **当 RAM 允许（更快的迭代）时，使用 `in_memory=True`** 用于 AlignTraj 
- **在分析之前选择最小原子**以减少内存/计算
- **使用多处理**进行独立帧分析
- **使用 `start`/`stop`/`step` 参数对很长的轨迹进行块处理**：
