...
1. **使用稀疏矩阵**：QuTiP 自动执行此操作
2. **最小化希尔伯特空间**：尽可能截断
3. **选择正确的求解器**：
  - 纯状态：`sesolve` 比 `mesolve`
 更快 - 随机：`mcsolve` 用于量子跳跃
 - 周期：Floquet 方法
4. **时间相关术语**：字符串格式fastest
5. **期望值**：仅计算所需的可观测值
6. **并行轨迹**：`mcsolve` 使用所有 CPU
7. **Krylov 方法**：对于非常大的系统
8. **内存**：尽可能使用`store_final_state`代替`store_states`
