---
name: aeon
description: 该技能应用于时间序列机器学习任务，包括分类、回归、聚类、预测、异常检测、分割和相似性搜索。当处理时态数据、顺序模式或时间索引观察需要标准 ML 方法之外的专用算法时使用。特别适合使用 scikit-learn 兼容 API 进行单变量和多变量时间序列分析。
...
Aeon 是一个与 scikit-learn 兼容的 Python 时间序列机器学习工具包。它提供了最先进的分类、回归、聚类、预测、异常检测、分割和相似性搜索算法。
...
将时间序列分类为预定义的类别。完整算法目录见`references/classification.md`。
...
从时间序列中预测连续值。算法参见`references/regression.md`。
...
将没有标签的相似时间序列分组。方法见`references/clustering.md`。
...
预测未来的时间序列值。预测者请参阅 `references/forecasting.md`。
...
识别异常模式或异常值。检测器请参见 `references/anomaly_detection.md`。
...
转换时间序列以进行特征工程。参见`references/transformations.md`.
...
- **锁步**：欧几里得、曼哈顿、Minkowski
- **基于形状**：形状 DTW、SBD

## 深度学习网络

时间序列的神经架构。参见`references/networks.md`.
...
加载标准基准测试并评估性能。请参见 `references/datasets_benchmarking.md`.
...
3. **比较基线**：针对简单方法（1-NN 欧几里得，朴素）
4 进行测试。 **考虑资源**：ROCKET 提高速度，深度学习（如果 GPU 可用）
...
* * 对于小数据集：**
- 基于距离：`KNeighborsTimeSeriesClassifier` DTW
- 避免：深度学习（需要大数据）
