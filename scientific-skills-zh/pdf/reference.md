...
本文档包含 PDF 的高级处理功能、详细示例以及主要技能说明中未涵盖的附加库。

## pypdfium2 库（Apache/BSD 许可证）

### 概述
pypdfium2 是 PDFium（Chromium 的）的 Python 绑定PDF 库）。它非常适合快速 PDF 渲染、图像生成，并可用作 PyMuPDF 替代品。
...
### 1.对于大型PDF
- 使用流式传输方法而不是在中加载整个PDF memory
- 使用 `qpdf --split-pages` 分割大文件
- 使用 pypdfium2

### 单独处理页面 2. 对于文本提取
- `pdftotext -bbox-layout` 纯文本提取最快
- 使用 pdfplumber 处理结构化数据和表格
- 对于非常大的文档，请避免使用 `pypdf.extract_text()`
