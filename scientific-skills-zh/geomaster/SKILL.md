---
name: geomaster
description: 全面的地理空间科学技能，涵盖遥感、GIS、空间分析、地球观测机器学习和 30 多个科学领域。支持卫星图像处理（Sentinel、Landsat、MODIS、SAR、高光谱）、矢量和栅格数据操作、空间统计、点云处理、网络分析、云原生工作流程（STAC、COG、行星计算机）和 8 种编程语言（Python、R、Julia、JavaScript、C++、Java、Go、Rust）以及 500 多个代码示例。用于遥感工作流程、GIS 分析、空间 ML、地球观测数据处理、地形分析、水文建模、海洋空间分析、大气科学和任何地理空间计算任务。
license: MIT License
metadata:
...
全面的地理空间科学技能，涵盖 GIS、遥感、空间分析和用于地球观测的 ML，涵盖 70 多个主题，以及 8 种编程语言的 500 多个代码示例。
...
|类型 |示例 |图书馆|
|-----|---------|-----------|
|矢量| Shapefile、GeoJSON、GeoPackage | GeoPandas，菲奥娜，GDAL |
|光栅| GeoTIFF、NetCDF、COG | Rasterio、Xarray、GDAL |
|点云|洛杉矶，洛杉矶 | Laspy、PDAL、Open3D |

### 坐标系

- **EPSG:4326** (WGS 84) - 地理、纬度/经度，用于存储
- **EPSG:3857** (Web 墨卡托) - 仅 Web 地图（不要用于区域/距离！）
- **EPSG:326xx/327xx** (UTM) - 公制计算，每个区域 <1% 失真
- 使用 `gdf.estimate_utm_crs()` 进行自动 UTM 检测
...
5. **使用高效格式**：GeoPackage > Shapefile、Parquet 用于大数据
6. **将云遮蔽**应用于光学图像
7. **保留谱系**以进行可重复的研究
8. **为您的分析比例使用适当的分辨率**
...
* *GeoMaster 涵盖从基本 GIS 操作到高级遥感和机器学习的所有内容。**
