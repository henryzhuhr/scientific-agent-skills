...
PufferLib 策略是标准 PyTorch 模块，具有用于观察处理和 LSTM 集成的可选实用程序。该框架提供了默认的架构和工具，同时允许策略设计的充分灵活性。
...
PufferLib提供`layer_init`以获得适当的权重初始化：
...
PufferLib 提供优化的 LSTM 集成与自动递归处理：
...
PufferLib 的 LSTM 优化在推出期间使用 LSTMCell，在训练期间使用 LSTM，速度提高了 3 倍推理：
